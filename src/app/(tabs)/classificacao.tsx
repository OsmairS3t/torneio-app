...
export default function Classification() {
  ...
  return (
    <View style={global.container}>
      <Header title="CLASSIFICAR" />
      
      <ScrollView>
        <View style={form.container}>
          <View style={form.groupHeader}>
            <SelectList
              placeholder="Torneio"
              boxStyles={form.selectMini}
              setSelected={(val: string) => setTournament(val)}
              onSelect={() => getClass(Number(tournament))}
              data={dataTournament}
              save="key"
            />
            <TouchableOpacity style={form.buttonIcon} onPress={() => getClass(Number(tournament))}>
              <Feather name='repeat' size={20} style={form.textButtonAddNew} />
            </TouchableOpacity>
          </View>
